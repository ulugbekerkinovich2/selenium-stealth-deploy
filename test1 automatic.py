...
for upload_data in range(0, len(arr[-1])):
    ...
    time.sleep(2)
    driver.find_element(By.XPATH, '/html/body/div[2]/div/div/div/div[2]/div/div/div[1]/div[3]/div/button[1]').click()
    time.sleep(2)
    ...
    for i in range(0, len(all_h3) + 1):
        ...
        if len(cycle) >= 2 or 'Файл' in cycle:
            time.sleep(0.5)
            driver.find_element(By.XPATH, '//*[@id="app"]/div[2]/div[2]/main/div[2]/div[2]/div/div[1]/button').click()
            time.sleep(3)
